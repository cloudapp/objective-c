...
+ (CLWebItem *)webItemWithAPIDictionary:(NSDictionary *)jsonDict
{
    ...
    webItem.thumbnailURL = [self _URLFromDictionary:jsonDict forKey:@"thumbnail_url"];
...
+ (NSURLRequest *)URLRequestWithS3ParametersDictionary:(NSDictionary *)s3Dict fileName:(NSString *)fileName fileData:(NSData *)fileData
{
    ...
    NSAssert([fileData isKindOfClass:[NSData class]], @"assert");
...
+ (NSURLRequest *)URLRequestWithS3ParametersDictionary:(NSDictionary *)s3Dict fileName:(NSString *)fileName filePathOnDisk:(NSString *)filePath {
    // Make sure to combine any decomposed pieces of any unicode characters (may be diacritical marks) into precomposed characters
    fileName = [fileName precomposedStringWithCanonicalMapping];
    ...
    PKMultipartInputStream *bodyInputStream = [[PKMultipartInputStream alloc] init];
    
    [actualParams enumerateKeysAndObjectsUsingBlock:^(NSString *key, NSString *value, BOOL *stop) {
        [bodyInputStream addPartWithName:key string:value];
    }];
    
    [bodyInputStream addPartWithName:@"file" path:filePath targetFileName:fileName];
    
    [request setValue:[NSString stringWithFormat:@"multipart/form-data; boundary=%@; charset=UTF-8", [bodyInputStream boundary]] forHTTPHeaderField:@"Content-Type"];
    [request setValue:[NSString stringWithFormat:@"%d", [bodyInputStream length]] forHTTPHeaderField:@"Content-Length"];
    [request setHTTPBodyStream:bodyInputStream];
    [request setHTTPMethod:@"POST"];
    
    request.HTTPBodyStream = bodyInputStream;
